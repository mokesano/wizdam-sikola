...
A: Sangia adalah backend analitik AI (`api.sangia.org`) yang melakukan kalkulasi berat: SDG classification, impact score, trend analysis, dan policy recommendation. Sangia tidak menyimpan data — semua persistensi dilakukan oleh Wizdam Sikola. Sangia Engine dikelola terpisah di repository `wizdam-apis`.
...
**Q: Kenapa WIZDAM_SHARED_SECRET harus sama antara Sikola dan APIs?**  
...
mysql -u root -p wizdam_sikola < database_migration_vX.sql
```
